...
#define setsize(newblock, size)	(*cast(unsigned int *, newblock) = size)
#define checkblocksize(b, size) (size == (*cast(unsigned int *, blockhead(b))))
...
static void *checkblock (void *block, unsigned int size) {
...
static void freeblock (void *block, unsigned int size) {
  if (block) {
...
void *debug_realloc (void *block, unsigned int oldsize, unsigned int size) {
  ...
  if (size == 0) {
    ...
    void *newblock;
    int i;
    unsigned int realsize = HEADER+size+MARKSIZE;
    unsigned int commonsize = (oldsize < size) ? oldsize : size;
...
static int newuserdata (lua_State *L) {
  unsigned int size = luaL_checkint(L, 1);
