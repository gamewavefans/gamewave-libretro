...
typedef long sint32;	/* a signed version for unsigned int */
...
static sint32 posrelat (sint32 pos, unsigned int len) {
...
static int str_byte (lua_State *L) {
  ...
  if (pos <= 0 || (unsigned int)(pos) > l)  /* index out of range? */
    return 0;  /* no answer */
...
static int str_find (lua_State *L) {
  ...
  if (init < 0) init = 0;
  else if ((unsigned int)(init) > l1) init = (sint32)l1;
...
static int gfind_aux (lua_State *L) {
  ...
  unsigned int ls = lua_strlen(L, lua_upvalueindex(1));
  const char *p = lua_tostring(L, lua_upvalueindex(2));
  const char *src;
  ms.L = L;
  ms.src_init = s;
  ms.src_end = s+ls;
  for (src = s + (unsigned int)lua_tonumber(L, lua_upvalueindex(3));
       src <= ms.src_end;
       src++) {
    const char *e;
    ms.level = 0;
